...
cities_sf3035<- st_transform(cities_sf4326, crs =  2035)
...
download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "C:/Users/cecil/OneDrive/AU/Spatial Analytics/Assignments/cds-spatial/Week03/inscriptions.json")

# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("C:/Users/cecil/OneDrive/AU/Spatial Analytics/Assignments/cds-spatial/Week03/inscriptions.json")
...
i_sm <- inscriptions %>% 
  ...
  mutate(latitude = gsub(")", "", latitude), latitude = gsub(" ", "", latitude),
         longitude = gsub("c", "", longitude), longitude = gsub("\\(", "", longitude),
...
leaflet() %>% addTiles() %>% addMarkers(lng=inscriptions2$longitude,lat=inscriptions2$latitude)
```
...
**Answer**
We want the same CRS, and we use EPSG as the value for CRS because it refers to both the datum and
projection
...
cities_5km2 <- st_transform(cities_5km, crs = 3035)
...
cities_it <- st_intersection(insc_ch, st_transform(cities_5km2))
