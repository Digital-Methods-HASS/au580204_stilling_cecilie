...
```{r}
Western_Europe <- rename(Western_Europe, "homicides" = "Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)" )
```


##Task 2: Other visualisation
...
ggplot(data = Western_Europe) + 
  geom_line(mapping = aes(x = Year, 
                          y = Western_Europe$`homicides`, 
                           color = Entity)) +
  facet_wrap(~ Entity, nrow = 3) + #use nrow to change how many rows
  labs(x = "Year",
       y = "Homicides per 100,000",
       title = "Homicide rate in Western Europe from 1300-2000")
```
...
##Task 5: Create table of contents and more
